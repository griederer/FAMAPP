// Event service for Firebase operations - shared between web and mobile
import { 
  collection, 
  doc, 
  addDoc, 
...
class EventService implements IEventService {
  ...
  async createEvent(data: CreateEventData): Promise<string> {
    ...
    try {
      ...
      const docRef = await addDoc(collection(this.db, this.COLLECTION_NAME), eventData);
      return docRef.id;
    } catch (error) {
  ...
  async updateEvent(id: string, data: UpdateEventData): Promise<void> {
    try {
      const eventRef = doc(this.db, this.COLLECTION_NAME, id);
