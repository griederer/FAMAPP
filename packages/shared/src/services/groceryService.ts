// Grocery service for Firebase operations - shared between web and mobile
import { 
  collection, 
  doc, 
  addDoc, 
...
import type { 
  GroceryItem, 
  GroceryList, 
  CreateGroceryListData, 
  CreateGroceryItemData, 
  UpdateGroceryItemData,
  GroceryStats,
  GroceryService as IGroceryService 
} from '../types/grocery';
...
class GroceryService implements IGroceryService {
  ...
  async createGroceryList(data: CreateGroceryListData): Promise<string> {
    ...
    try {
      ...
      const docRef = await addDoc(collection(this.db, this.LISTS_COLLECTION), cleanData);
      return docRef.id;
    } catch (error) {
  ...
  async updateGroceryList(id: string, data: Partial<CreateGroceryListData>): Promise<void> {
    try {
      const listRef = doc(this.db, this.LISTS_COLLECTION, id);
  ...
  async deleteGroceryList(id: string): Promise<void> {
    try {
      ...
      const itemsQuery = query(
        collection(this.db, this.ITEMS_COLLECTION),
        where('listId', '==', id)
      );
      ...
      await deleteDoc(doc(this.db, this.LISTS_COLLECTION, id));
    } catch (error) {
  ...
  async toggleGroceryListCompletion(id: string): Promise<void> {
    try {
      ...
      // Get current state
      const listSnapshot = await getDocs(query(collection(this.db, this.LISTS_COLLECTION), where('__name__', '==', id)));
  ...
  async addGroceryItem(listId: string, data: CreateGroceryItemData): Promise<string> {
    ...
    try {
      ...
      const docRef = await addDoc(collection(this.db, this.ITEMS_COLLECTION), cleanData);
      
      // Update list timestamp
      await updateDoc(doc(this.db, this.LISTS_COLLECTION, listId), {
        updatedAt: serverTimestamp(),
      });
  ...
  async updateGroceryItem(id: string, data: UpdateGroceryItemData): Promise<void> {
    ...
    try {
      ...
      await updateDoc(doc(this.db, this.ITEMS_COLLECTION, id), updateData);
    } catch (error) {
  ...
  async deleteGroceryItem(id: string): Promise<void> {
    try {
      await deleteDoc(doc(this.db, this.ITEMS_COLLECTION, id));
    } catch (error) {
  ...
  subscribeToGroceryLists(callback: (lists: GroceryList[]) => void): Unsubscribe {
    const q = query(
      collection(this.db, this.LISTS_COLLECTION),
    ...
    return onSnapshot(q, async (snapshot) => {
      try {
        ...
        if (error.code === 'failed-precondition' && error.message.includes('index')) {
          ...
          const fallbackQ = query(collection(this.db, this.LISTS_COLLECTION));
  ...
  // Get shopping stats
  async getShoppingStats(): Promise<GroceryStats> {
    try {
      const listsSnapshot = await getDocs(collection(this.db, this.LISTS_COLLECTION));
      const itemsSnapshot = await getDocs(collection(this.db, this.ITEMS_COLLECTION));
